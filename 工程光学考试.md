...
激光器输出的光束经$L_1$扩束，针孔$H$滤波，分光版G反射后被$L_2$准直称平行光束垂直入射到待测零件$Q$上。$Q$置于可微调的平台上。从待测平面的上下表面分别反射的光返回经过$G$进入观察系统$L_3$，可测量平板的平行度。此时观察到的是待测零件上下表面形成的等厚干涉条纹。若平行平板的两表面平行，则会看到视场中一片均匀；若平行平板的两表面不严格平行，则会看到等间隔的等厚干涉直条纹。
...
- 特征参数
  ...
  - 在主轴系下椭圆偏振光的长轴在制定坐标系中的方位角、椭圆度和旋向
